...
public abstract class RepositoryPatternBase {
    ...
    public RepositoryPatternBase (String URI){
    }
    ...
    /*
     *  Staging rules:
     *      Always take away from a table when adding txn to staging table
     *      Never add to main tables when adding txn to staging table
     *
     *  Committing rules:
     *      All issues taken care of in staging table, should never have a commit issue
     *
     *  ClientID    ticker  amount
     *      1       CASH    $100
     *      1       AAPL    100
     *
     *
     ...
     *
     *  Broker Staging table:
     *      txnID   ticker  amount  B/S
     *
     *  Client Staging table:
     *      txnID   ticker  amount  B/S
     *         1    AAPL    10      B
     *         1    CASH    10      B
     */
    ...
    /**
     * Attempts to place buy on broker staging table, only subtracting from main Broker table
     * If fails, adds back removed
     * @return true IFF succeed
     */
    protected abstract boolean stageBrokerBuy(TransactionDetails tx);
    /**
     *  Attempts to place client buy on staging table, only subtracting from main client table
     *  if fails adds back removed
     * @return true IFF succeed
     */
    ...
    /**
     * removed Buy from broker table and place in main DB
     * adds txn to txnTable
     */
    protected abstract void commitBrokerBuy(TransactionDetails tx);

    /**
     * Removes Buy from client staging and makes alterations to both client rows in main DB
     * adds tx to txnTable
     */
    protected abstract void commitClientBuy(TransactionDetails tx);

    /**
     * Attempts to place sell on broker staging table, only subtracting from main Broker table
     * If fails, adds back removed
     * @return true IFF succeed
     */
    protected abstract boolean stageBrokerSell(TransactionDetails tx);
    /**
     *  Attempts to place client sell on staging table, only subtracting from main client table
     *  if fails adds back removed
     * @return true IFF succeed
     */
