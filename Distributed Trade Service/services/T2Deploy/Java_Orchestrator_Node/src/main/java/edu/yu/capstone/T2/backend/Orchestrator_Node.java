...
import edu.yu.capstone.tradeServer.t3.observe.EventManager;
import edu.yu.capstone.tradeServer.t3.observe.MockKafka;
...
public class Orchestrator_Node {
    ...
    private final Producer<Long, TransactionDetails> kafkaProducer;
...
//    private final EventManager eventManager;
    ...
    private class DeliveredTuple {
        ...
        protected TransactionDetails getDetails(){return details;}
        protected long getDeliveryTag() {return deliveryTag;}
    }
    ...
    public Orchestrator_Node(ConnectionFactory rabbitBus,
                             String tradeServiceQueue,
                             String brokerInvQueue,
                             String clientAccQueue,
                             String generalRoutingKey,
                             KafkaProducer<Long, TransactionDetails> kafkaProducer,
...
//        this.eventManager = new EventManager(mockKafka, "machine-1");
    ...
    public void run(){
        ...
        DeliverCallback tradeCallBack = (consumerTag, delivery) -> {
            DeliveredTuple tuple = new DeliveredTuple(
                    TransactionDetails.deserialize(delivery.getBody()),
                    delivery.getEnvelope().getDeliveryTag());
            System.out.printf("Message received from: %s \n", tradeServiceQueue);
//            eventManager.sendEvent("T2", tuple.details.getTransactionID(), EventType.TRANSACTION_START.name(), System.currentTimeMillis());
            ...
            try {
                ...
                System.out.printf("error placing a message on the waitingMessages from tradeQueue, txnID: %d cancelled\n", TransactionDetails.deserialize(delivery.getBody()).getTransactionID());
            }
        };
        DeliverCallback receiverCallBack = (consumerTag, delivery) -> {
            ...
            try {
                ...
                System.out.printf("error placing a message on the waitingMessages from receiverQueue, txnID: %d cancelled\n", TransactionDetails.deserialize(delivery.getBody()).getTransactionID());
            }
        };


        //basicConsume is a non-blocking method that spawns its own thread to continuosly consume messages
        try {
            //start consuming
            tradeChannel.basicConsume(tradeServiceQueue, false, tradeCallBack, consumerTag -> {});
            nodeChannel.basicConsume(orchestratorReturnQueue, true, receiverCallBack, consumerTag -> {}); //Auto-ack true here
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
...
//                    System.out.printf("Received tx, txnID: %d, clientID: %d, %s\n", details.getTransactionID(), details.getClientID(), details.getTransactionType().toString());
...
//        transactionExecutor.submit(cleanup);
    ...
    /**
     * This method should be periodically run.
     * It goes through the list of timer futures in the order they were added to the list.
     *  If Completed
     *      Normally -> no action
     *      TimerError -> there was a timeout in the tx, something is wrong
     *      ExceuctionException or RuntimeException -> unintended path, needs error handling
     *  If not Completed
     *      -> Break out of loop as timers are added in order
    ...
    private void cleanupTimers() throws InterruptedException {
        if (!timers.isEmpty()) {
            ...
            while (it.hasNext()) {
                //only goes through the uncompleted list, as only really checking for timers that have timed out
    ...
    /**
     * confirms with the rabbit worker queue to remove the transaction from the equation
     */
...
//        eventManager.sendEvent("T2", unit.getDetails().getTransactionID(), EventType.TRANSACTION_END.name(), System.currentTimeMillis());

        //TODO integrate kafka in
//        TransactionDetails transaction = unit.getDetails();
//        CompletableFuture<RecordMetadata> completableFuture = new CompletableFuture<>();
//
//        ProducerRecord<Long, TransactionDetails> record = new ProducerRecord<>(kafkaTopicName, transaction.getTransactionID(), transaction);
//
...
//        }
    ...
    /**
     * submits the provided message to the queues between the orchestrator and the mongoNodes
    ...
    /**
     * increments a recovery internally, if fully recovered, posts a failure transaction to kafka
     */
